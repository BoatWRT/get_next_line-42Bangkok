...
/*   Updated: 2023/10/19 18:33:59 by <wtangcha>       ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static char	*full_str[404];
	char		*line;

	if (fd < 0 || BUFFER_SIZE <= 0)
		return (NULL);
	full_str[fd] = ft_read_first_line(fd, full_str[fd]);
	if (!full_str[fd])
		return (NULL);
	line = ft_get_first_line(full_str[fd]);
	full_str[fd] = ft_get_rest_line(full_str[fd]);
	return (line);
}
